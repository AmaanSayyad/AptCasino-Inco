...
const QUICK_BET_PRESETS = [0.5, 1, 2, 5, 10];
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex gap-1.5 p-1 rounded-lg bg-[#120521] border border-[#333947]/80">
        <button type="button" onClick={() => onModeChange('reset')} className={cn('flex-1 rounded-md py-2 text-xs font-medium transition-all', mode === 'reset' ? 'bg-gradient-to-r from-[#F1324D]/90 to-[#2414E3]/90 text-white shadow-sm' : 'text-white/45 hover:text-white/70')}>Reset bet</button>
        <button type="button" onClick={() => onModeChange('increase')} className={cn('flex-1 rounded-md py-2 text-xs font-medium transition-all', mode === 'increase' ? 'bg-gradient-to-r from-[#F1324D]/90 to-[#2414E3]/90 text-white shadow-sm' : 'text-white/45 hover:text-white/70')}>Increase</button>
      </div>
      {mode === 'increase' && (
...
/**
 * Faithful port of the original BettingPanel — dropped the old multi-chain
 * currency switcher (this app is USDC-only now) and the client-side balance
 * store; balance/bet state comes from useConfidentialGame(). Auto-bet now
 * runs a real sequential loop — each round is a genuine on-chain confidential
 * round (approve -> play -> Inco reveal -> settle), so it's slower than the
 * original's instant simulation, but every round is real.
 */
const BettingPanel = ({ wager, setWager, risk, setRisk, noOfSegments, setSegments, busy, onManualBet, onAutoBet, usdcBalance }) => {
  ...
  const buttonLabel = busy ? (gameMode === 'auto' ? 'Auto spinning…' : 'Spinning…') : gameMode === 'auto' ? 'Start Autobet' : 'Start Bet';
    ...
    <div className="flex h-full flex-col rounded-3xl border border-[#333947]/80 bg-gradient-to-b from-[#290023] to-[#150012] p-4 shadow-xl shadow-black/30">
      <div className="mb-5 flex rounded-xl border border-[#333947]/60 bg-[#120521] p-1">
        {['manual', 'auto'].map((mode) => (
          <button key={mode} type="button" onClick={() => setGameMode(mode)} className={cn('flex-1 rounded-lg py-2.5 text-sm font-medium capitalize transition-all', gameMode === mode ? 'bg-gradient-to-r from-[#F1324D] to-[#2414E3] text-white shadow-md' : 'text-white/40 hover:text-white/70')}>{mode}</button>
        ))}
      </div>
      ...
      <div className="flex flex-1 flex-col gap-4 overflow-y-auto">
        <div>
          <div className="mb-2 flex items-center justify-between gap-2">
            ...
            <span className="text-[11px] text-white/45 tabular-nums">{balance.toFixed(2)} USDC</span>
          </div>
          ...
          <div className="mt-2 grid grid-cols-5 gap-1">
            {QUICK_BET_PRESETS.map((preset) => (
              <button key={preset} type="button" disabled={busy} onClick={() => applyBet(preset)} className={cn('rounded-lg border py-1.5 text-[10px] font-medium transition-colors disabled:opacity-50', Math.abs(parsedBet - preset) < 0.0001 ? 'border-purple-400/50 bg-purple-600/25 text-white' : 'border-[#333947]/50 bg-[#120521] text-white/60 hover:border-purple-500/30 hover:text-white')}>{preset}</button>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {RISK_OPTIONS.map(({ id, label }) => (
              <button key={id} type="button" disabled={busy} onClick={() => setRisk(id)} className={cn('rounded-lg border py-2.5 text-xs font-medium transition-all disabled:opacity-50', risk === id ? 'border-purple-400/60 bg-purple-600/20 text-white' : 'border-[#333947]/60 bg-[#120521] text-white/50 hover:border-purple-500/30 hover:text-white/80')}>{label}</button>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="relative">
            <select value={noOfSegments} disabled={busy} onChange={(e) => setSegments(parseInt(e.target.value, 10))} className="w-full appearance-none rounded-xl border border-[#333947]/80 bg-[#120521] px-3 py-2.5 text-sm text-white outline-none focus:border-purple-500/40 disabled:opacity-50">
              {SEGMENT_OPTIONS.map((n) => <option key={n} value={n}>{n} segments</option>)}
            </select>
        ...
        {gameMode === 'auto' && (
          <div className="space-y-4 rounded-xl border border-[#333947]/50 bg-[#120521]/60 p-3">
            ...
            <p className="text-[10px] leading-relaxed text-white/35">Leave stop fields at 0 to disable. Each round settles fully on-chain before the next starts, so an auto session is much slower than the old simulated one — that's the real confidential-compute round trip, not a bug.</p>
          </div>
        )}
      </div>
      ...
      <div className="mt-4 space-y-2">
        ...
        <button type="button" onClick={handleSubmit} disabled={!canBet} className={cn('w-full rounded-xl py-3.5 text-center text-sm font-semibold transition-all', canBet ? 'bg-gradient-to-r from-[#F1324D] to-[#2414E3] text-white shadow-lg shadow-purple-900/30 hover:brightness-110 active:scale-[0.99]' : 'cursor-not-allowed bg-[#333947]/40 text-white/30')}>{buttonLabel}</button>
      </div>
    </div>
  );
};
