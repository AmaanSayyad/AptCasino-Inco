...
/**
 * Faithful port of the original canvas wheel renderer. The only behavioral
 * change: the landed segment always comes from `forcedSegmentIndex` (the
 * real on-chain outcome.segment from AptCasino.sol's settle()) — there is no
 * client-side probability guess anymore, since the actual result is only
 * known once the confidential round settles.
 */
const GameWheel = ({
  isSpinning,
  noOfSegments,
  ...
  const wheelData = useMemo(() => buildExpandedWheelSegments(risk, noOfSegments), [risk, noOfSegments]);
  const segments = wheelData.length;
  const panelMultipliers = useMemo(() => wheelPanelMultipliers(risk, noOfSegments), [risk, noOfSegments]);
  const panelColorMap = useMemo(() => wheelPanelColorMap(risk, noOfSegments), [risk, noOfSegments]);
  ...
  const pointerIndex = !isSpinning && segments > 0 ? segmentIndexUnderPointer(wheelPosition, segments) : null;
  const activeIndex = isSpinning ? null : pointerIndex;
  const currentSegment = activeIndex != null ? wheelData[activeIndex] : wheelData[pointerIndex ?? 0];
  ...
  useEffect(() => {
    ...
    const startAnimation = () => {
      ...
      const selectedIndex = forced != null && forced >= 0 ? forced % segments : 0;
      const totalSpins = 5;
      const targetPosition = wheelRotationForSegmentIndex(selectedIndex, segments);
      const startRotation = ((wheelPositionRef.current % (Math.PI * 2)) + Math.PI * 2) % (Math.PI * 2);
      ...
      const animate = (timestamp) => {
        ...
        const landedIndex = forced != null && forced >= 0 ? forced % segments : segmentIndexUnderPointer(finalPosition, segments);
        const landed = wheelData[landedIndex];
        handleSelectMultiplierRef.current?.(landed.multiplier);

        const payload = { segmentIndex: landedIndex, multiplier: landed.multiplier, color: landed.color, probability: landed.probability };
  ...
  return (
    <div className="flex flex-col justify-between items-center h-full w-full">
      ...
      <div className="relative flex h-[435px] w-[600px] sm:h-[525px] sm:w-[500px] lg:h-[625px] lg:w-[600px] items-center justify-center p-4">
        <Image src={arrow} width={50} height={50} alt="Pointer Arrow" className="absolute top-0 left-1/2 -translate-x-1/2 z-10" />
        <canvas ref={canvasRef} className="max-w-[85vw] max-h-[85vh] rounded-full pt-4 p-2 bg-[#0A0009]" />

        {!isSpinning && hasSpun && activeIndex != null && (
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            <div className="text-4xl font-bold text-white animate-bounce mb-2">{currentSegment.multiplier.toFixed(2)}x</div>
            {currentSegment.multiplier > 0 && (
              <div className="text-lg text-green-400 bg-black/50 px-3 py-1 rounded-lg">Winnings: {currentSegment.multiplier.toFixed(2)} × bet</div>
            )}
          </div>
        )}
      </div>
        ...
        <div className="p-4 bg-[#1a1a1a] rounded-lg border border-[#333947]">
          <div className="text-center">
            ...
            <div className="flex items-center justify-center gap-4">
              <div className="w-8 h-8 rounded-full border-2 border-white" style={{ backgroundColor: currentSegment.color }} />
              <div className="text-2xl font-bold text-white">{currentSegment.multiplier.toFixed(2)}x</div>
              <div className="text-sm text-gray-400">({(currentSegment.probability * 100).toFixed(1)}% chance)</div>
            </div>
            {currentSegment.multiplier > 0 && hasSpun && !isSpinning && (
              <div className="mt-2 p-2 bg-green-900/20 border border-green-700/30 rounded-md">
                <div className="text-sm text-green-300">Bet × {currentSegment.multiplier.toFixed(2)} = Winnings</div>
              </div>
            )}
            {activeIndex != null && <div className="text-xs text-gray-500 mt-2">Segment {activeIndex + 1} of {segments}</div>}
          </div>
        </div>
      </div>
      ...
      <div className="flex w-full gap-3 p-2">
        {panelMultipliers.map((multiplier) => {
          const isSelected = hasSpun && activeIndex != null && currentSegment.multiplier === multiplier;
          const bgColor = panelColorMap[multiplier] || '#333947';
          return (
            <div key={multiplier} className="flex flex-col justify-end items-center h-[60px] w-full rounded-md text-sm font-medium border bg-[#0A0009] border-[#333947] transition-all" style={isSelected ? { backgroundColor: bgColor } : {}}>
