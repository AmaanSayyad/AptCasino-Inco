...
const stageCopy = {
  idle: 'Ready', approving: 'Approving USDC', betting: 'Locking wager on Base', revealing: 'Waiting for Inco covalidators', settling: 'Verifying attestation on-chain', done: 'Settled', error: 'Action needed',
};
...
export default function ConfidentialGame({ game }) {
  ...
  async function play() {
    ...
    try {
      ...
      let args;
      let outcomeEvent;
      if (game === 'roulette') { functionName = 'playRoulette'; args = [betType, selection, wagerRaw]; outcomeEvent = 'RouletteOutcome'; }
      if (game === 'wheel') { functionName = 'playWheel'; args = [risk, segments, wagerRaw]; outcomeEvent = 'WheelOutcome'; }
      if (game === 'plinko') { functionName = 'playPlinko'; args = [risk, rows, wagerRaw]; outcomeEvent = 'PlinkoOutcome'; }
      if (game === 'mines') { functionName = 'playMines'; args = [tiles, mineCount, wagerRaw]; outcomeEvent = 'MinesOutcome'; }
      const response = await runConfidentialGame({ account: address, functionName, args, wager: wagerRaw, outcomeEvent, onStage: setStage });
      setResult(response);
      creditsRead.refetch();
    } catch (playError) {
      setStage('error');
        ...
        <section className="overflow-hidden rounded-3xl bg-gradient-to-r from-red-magic to-blue-magic p-[1px] shadow-2xl shadow-fuchsia-950/30">
          ...
          <div className="grid gap-8 p-6 lg:grid-cols-[1fr_340px] lg:p-10">
            <div>
              ...
              <div className="mt-6"><Field label="Wager (USDC)"><input className="game-input" type="number" min="0.1" max="10" step="0.1" value={wager} onChange={(e) => setWager(e.target.value)} /></Field></div>
              <div className="mt-6">
                {!isConnected ? <ConnectWalletButton /> : <button onClick={play} disabled={['approving','betting','revealing','settling'].includes(stage)} className="rounded-xl bg-white px-7 py-3 font-black text-black transition hover:bg-white/85 disabled:cursor-wait disabled:opacity-50">{stage === 'idle' || stage === 'done' || stage === 'error' ? 'Play confidential round' : stageCopy[stage]}</button>}
              </div>
              {error && <p className="mt-4 rounded-xl border border-red-400/30 bg-red-400/10 p-4 text-sm text-red-100">{error}</p>}
              {outcome && <div className="mt-7 rounded-2xl border border-emerald-400/25 bg-emerald-400/10 p-5"><p className="text-xs font-bold uppercase tracking-widest text-emerald-300">Verified result</p><p className="mt-2 text-2xl font-black">{resultLabel(game, outcome)}</p><p className="mt-1 text-white/65">Payout: {payout} USDC</p><a className="mt-3 inline-block text-xs text-emerald-300 hover:underline" href={`https://sepolia.basescan.org/tx/${result.settleHash}`} target="_blank" rel="noreferrer">View settlement on BaseScan ↗</a></div>}
            </div>
            <aside className="space-y-4">
              <div className="rounded-2xl border border-white/10 bg-black/20 p-5"><p className="text-xs font-bold uppercase tracking-widest text-white/45">Round status</p><p className="mt-2 font-black">{stageCopy[stage]}</p><div className="mt-4 h-2 overflow-hidden rounded-full bg-white/10"><div className={`h-full bg-gradient-to-r ${copy.accent} transition-all`} style={{ width: `${{idle:0,approving:14,betting:32,revealing:64,settling:84,done:100,error:100}[stage]}%` }} /></div></div>
              ...
              <Link href="/fairness" className="block rounded-2xl border border-white/10 p-5 text-sm text-white/65 transition hover:bg-white/5 hover:text-white"><strong className="block text-white">How privacy works</strong><span className="mt-1 block">Read how Inco + Megapot keep rounds provably fair →</span></Link>
            </aside>
          </div>
          </div>
        </section>
