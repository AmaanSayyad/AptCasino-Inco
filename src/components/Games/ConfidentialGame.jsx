...
const COPY = {
  roulette: { title: 'Confidential Roulette', intro: 'Pick a bet. Inco seals the winning number until your wager is locked.', accent: 'from-red-500 to-rose-700' },
  wheel: { title: 'Confidential Wheel', intro: 'The landing segment is encrypted before the wheel starts moving.', accent: 'from-violet-500 to-fuchsia-700' },
  plinko: { title: 'Confidential Plinko', intro: 'Inco chooses the hidden path; the board replays the verified bucket.', accent: 'from-cyan-500 to-blue-700' },
  mines: { title: 'Confidential Mines', intro: 'Select tiles before the encrypted board is revealed. No browser-generated bombs.', accent: 'from-amber-400 to-orange-700' },
};
  ...
  return (
    <main className="min-h-screen bg-[#080812] px-4 py-10 text-white">
      <div className="mx-auto max-w-6xl">
        ...
        <section className="overflow-hidden rounded-3xl border border-white/10 bg-[#10101c] shadow-2xl shadow-black/30">
          <div className={`bg-gradient-to-r ${copy.accent} px-6 py-10 sm:px-10`}>
            <p className="text-xs font-black uppercase tracking-[0.24em] text-white/70">AptCasino</p>
            <h1 className="mt-3 font-display text-4xl font-black sm:text-5xl">{copy.title}</h1>
            <p className="mt-3 max-w-2xl text-white/80">{copy.intro}</p>
          </div>
