...
// Every AptCasino.sol payout formula uses the same 97% factor (see
// src/lib/inco/payoutMath.js) — one honest RTP for all 4 real games,
// not the original's per-game numbers tuned for a different contract.
const RTP = '97.0%';

const LIVE_GAMES = [
  { status: 'live', slug: 'roulette', name: 'Roulette', img: '/images/games/roulette.png', link: '/game/roulette', categories: ['featured', 'table'], rtp: RTP },
  { status: 'live', slug: 'plinko', name: 'Plinko', img: '/images/games/plinko.png', link: '/game/plinko', categories: ['featured', 'instant'], rtp: RTP },
  { status: 'live', slug: 'mines', name: 'Mines', img: '/images/games/mines.png', link: '/game/mines', categories: ['featured', 'instant'], rtp: RTP },
  { status: 'live', slug: 'wheel', name: 'Spin Wheel', img: '/images/games/spin_the_wheel.png', link: '/game/wheel', categories: ['featured', 'instant'], rtp: RTP },
];

/** Not on-chain yet — shown as "Upcoming", no fabricated player counts. */
const UPCOMING_GAMES = [
  { status: 'upcoming', slug: 'fortune-tiger', name: 'Fortune Tiger', img: '/images/games/fortune-tiger.png', categories: ['slots', 'jackpot'], rtp: '—' },
  { status: 'upcoming', slug: 'poker', name: 'Poker', img: '/images/games/poker.png', categories: ['card', 'table'], rtp: '—' },
  { status: 'upcoming', slug: 'crash', name: 'Crash', img: '/images/games/crash.png', categories: ['instant', 'featured'], rtp: '—' },
  { status: 'upcoming', slug: 'gates-of-olympus', name: 'Gates of Olympus', img: '/images/games/gates-of-olympus.png', categories: ['slots', 'featured'], rtp: '—' },
  { status: 'upcoming', slug: 'carp-diem', name: 'Carp Diem', img: '/images/games/Carp_diem.png', categories: ['slots'], rtp: '—' },
  { status: 'upcoming', slug: 'fire-portal', name: 'Fire Portal', img: '/images/games/fire_portal.png', categories: ['instant'], rtp: '—' },
  { status: 'upcoming', slug: 'revenge-of-loki', name: 'Revenge of Loki', img: '/images/games/revenge_of_loki.png', categories: ['slots'], rtp: '—' },
  { status: 'upcoming', slug: 'sugar-rush', name: 'Sugar Rush', img: '/images/games/sugar_rush.png', categories: ['slots', 'jackpot'], rtp: '—' },
  { status: 'upcoming', slug: 'fire-in-the-hole', name: 'Fire in the Hole', img: '/images/games/fire_in_the_hole.png', categories: ['slots'], rtp: '—' },
  { status: 'upcoming', slug: 'dices', name: 'Dices', img: '/images/games/dices.png', categories: ['table', 'instant'], rtp: '—' },
];
...
/** Real bet counts from the live game_play_events table — no fabricated "players online". */
  ...
  useEffect(() => {
    ...
    fetch('/api/games/stats')
      ...
      .then((j) => {
        if (cancelled) return;
        const map = {};
        for (const row of j.stats || []) map[row.game] = row;
        setByGame(map);
      })
      .catch(() => {});
...
export default function MostPlayed() {
  ...
  const enriched = useMemo(() => {
    const live = LIVE_GAMES.map((g) => ({
      ...g,
      playersOnline: 0,
      totalBets: gameStats[g.slug]?.bets ?? 0,
    ...
    <section className="container mx-auto px-3 sm:px-4 pt-2 md:pt-4 pb-10 md:pb-16 relative">
      ...
      <div className="mb-5 md:mb-12 text-center max-w-3xl mx-auto">
        <HeaderText
          ...
          description="Live games settle on Base Sepolia with Inco Lightning confidential randomness. Other titles are marked Upcoming until they ship."
        />
      </div>
            ...
            <div className="flex gap-3 items-center">
              ...
              <div className="min-w-0 flex-1 text-left">
                ...
                <p className="mt-1 flex flex-wrap gap-x-2 gap-y-0.5 text-[11px] text-white/55">
                  <span>{currentFeatured.totalBets.toLocaleString()} bets all-time</span>
                  <span>·</span>
                ...
                <div className="flex flex-col md:flex-row gap-6 md:gap-10 items-center">
                  ...
                  <div className="md:w-2/3 text-center md:text-left">
                    ...
                    <p className="text-white/80 mb-6 max-w-2xl">
                      Experience {currentFeatured.name} — Inco-verified confidential randomness, attested
      ...
      <div className="transition-opacity duration-300">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-5 lg:gap-6">
          {visibleGames.map((game) => {
            ...
            const CardTag = isUpcoming ? 'div' : Link;
            const cardProps = isUpcoming ? { title: 'Coming soon' } : { href: game.link };
            return (
              <div
                key={game.slug}
                className={`group relative flex flex-col transition-all duration-300 md:hover:translate-y-[-8px] ${isUpcoming ? 'opacity-95' : ''}`}
              >
                <CardTag {...cardProps} className={`block w-full ${isUpcoming ? 'cursor-default' : ''}`}>
                  <MagicBorder>
                  ...
                  <div className="mt-2 md:mt-3 flex flex-col items-center">
                    ...
                    <p className="text-[10px] sm:text-xs text-white/55 mt-0.5 text-center px-0.5 line-clamp-2">
                      {isUpcoming ? 'Not on-chain yet — launching soon' : `${game.totalBets.toLocaleString()} bets all-time`}
                    </p>
                    <span
