...
export default function LivePage() {
  ...
  const wallet = connected && play.address ? play.address : null;
  ...
  const loadStreams = useCallback(async () => {
    try {
      ...
      const list = (Array.isArray(json.streams) ? json.streams : []).map((s) => ({
        ...s,
        playbackId: s.playback_id,
        thumbnailUrl: s.thumbnail_url,
        isLive: s.session_status === "live",
        durationMinutes: Math.round((s.duration_seconds || 0) / 60),
        rewardTierPct: s.reward_tier_pct || 0,
        xHandle: s.x_handle,
  ...
  const myLiveSession = streams.find(
    (s) => s.session_status === "live" && wallet && s.wallet && walletsMatch(s.wallet, wallet),
  );
  ...
  async function addStream() {
    ...
    if (!id) return;
    if (!connected || !wallet) {
      setErrorModal({ open: true, message: "Connect your wallet to go live." });
      return;
    }
    if (myLiveSession) {
    ...
    try {
      ...
      const res = await fetch("/api/streams", {
        ...
        body: JSON.stringify({
          wallet,
          playbackId: id,
          source: yt ? "youtube" : /^https?:\/\//i.test(id) ? "hls" : "livepeer",
          thumbnailUrl,
          xHandle: xHandle.trim() || null,
          telegramUsername: telegramUsername.trim() || null,
          payoutWallet: payoutWallet.trim() || null,
        }),
      });
      const json = await res.json().catch(() => ({}));
      if (!res.ok) {
  ...
  // Note: YouTube oEmbed preview metadata isn't wired up (no /api/youtube/oembed route
  // in this build) — ytMeta stays empty and StreamCard falls back to its own title/thumbnail.
      ...
      <div className="site-page-top site-page-pad-x min-h-[100dvh] bg-[#070005] pb-[max(4rem,env(safe-area-inset-bottom))] md:min-h-screen md:pb-16">
        <div className="bg-[#070005]/90 border border-purple-500/20 rounded-2xl p-4 md:p-6 shadow-xl backdrop-blur mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 p-4 rounded-xl border border-purple-500/20 bg-white/5">
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    placeholder="Payout wallet for rewards (optional)"
          ...
          {!connected && (
            <p className="mt-2 text-xs text-white/50">
              Connect a wallet in the header, then go live.
            </p>
          )}
          {connected && wallet && (
            <p className="mt-2 text-xs text-white/50">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5 md:gap-6">
          ...
          {streams.map((stream, idx) => {
            ...
            const isOwner = id && ownerWallet && wallet && walletsMatch(ownerWallet, wallet);
            ...
            <div className="bg-[#0e0010] text-white/90 p-6 md:p-7 max-h-[80vh] overflow-y-auto">
              ...
              <div className="rounded-xl border border-amber-500/25 bg-amber-500/10 px-4 py-3 mb-5 text-sm">
                ...
                <p className="text-white/45 text-xs mt-2">
                  ...
                  and sent to your payout wallet (if set).
                </p>
              </div>
