...
function normalizeAllocation(row) {
  ...
  const cliffEndsAt = new Date(lockedAtMs + row.cliff_days * 86_400_000).toISOString();
  const unlockAt = row.unlock_at;
  const now = Date.now();
  const secondsUntilCliff = Math.max(0, Math.round((new Date(cliffEndsAt).getTime() - now) / 1000));
  const secondsUntilUnlock = Math.max(0, Math.round((new Date(unlockAt).getTime() - now) / 1000));
  const effectiveStatus = row.status === 'locked' && secondsUntilUnlock <= 0 ? 'ready' : row.status;
  return {
    slug: row.kol_slug,
    displayName: row.display_name,
    walletAddress: row.wallet_address,
    amountAptc: row.amount_aptc,
    pctOfSupply: row.pct_of_supply,
    cliffDays: row.cliff_days,
    ...
    country: row.country,
    avgPostViews: row.avg_post_views,
    promotionCondition: row.promotion_condition,
...
const STATUS = {
  ...
  ready: {
    ...
    body: 'Your lock period has ended. AptCasino ops will send your token allocation to your registered wallet shortly.',
    tone: 'emerald',
    pill: 'Ready for payout',
    icon: FaUnlock,
  },
  fulfilled: {
    title: 'Allocation delivered',
...
export default function KolPortalClient({ slug }) {
  ...
  const login = async (e) => {
    ...
    try {
      ...
      setAllocation(normalizeAllocation(j.allocation));
      setAuthed(true);
      setPassword('');
    } catch (err) {
        ...
        <div className="rounded-2xl p-[1px] bg-gradient-to-br from-fuchsia-500/40 via-violet-500/20 to-transparent">
          <div className="rounded-2xl bg-[#0c0510]/95 p-8 shadow-2xl backdrop-blur">
            <div className="text-center mb-8">
              ...
              <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-fuchsia-300/80">KOL partner program</p>
  ...
  const statusBody =
    statusKey === 'locked'
      ? `Your partner allocation is reserved. It unlocks after ${lockTermsCopy(allocation)}, then the team processes your payout.`
      : status.body;
  ...
  const earningsTracks = [
    ...
    },
    {
      ...
      desc: 'Earn a share of qualified first deposits via your referral link.',
      href: '/referral',
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-amber-500/10 to-transparent p-5">
          ...
          <p className="text-sm text-amber-200/70 mt-2">allocated units</p>
        </div>
        ...
        <dl className="divide-y divide-white/5 text-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 px-5 py-4">
            ...
            <dd className="flex items-center gap-2 min-w-0">
              ...
              <a
                href={basescanUrl('address', allocation.walletAddress)}
                target="_blank"
                rel="noreferrer"
          ...
          {allocation.fulfillmentTxHash ? (
            <div className="flex justify-between gap-4 items-center px-5 py-4">
              ...
              <dd>
                <a
                  href={basescanUrl('tx', allocation.fulfillmentTxHash)}
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-1.5 text-cyan-300 hover:text-cyan-200 font-mono text-xs"
                >
                  View on BaseScan <FaExternalLinkAlt className="text-[10px] opacity-70" />
                </a>
              </dd>
            </div>
          ) : null}
      ...
      <section className="rounded-2xl border border-white/10 bg-black/20 p-5 sm:p-6">
        ...
        <p className="text-xs text-white/50 mb-5 max-w-2xl leading-relaxed">
          Stack multiple earning tracks — streams and referrals — to maximize total partner revenue beyond your locked allocation.
        </p>
